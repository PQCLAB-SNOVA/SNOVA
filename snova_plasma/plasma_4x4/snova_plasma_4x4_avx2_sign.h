...
int sign_digest_core_4x4_avx2_vtl(uint8_t *pt_signature, const uint8_t *digest, uint64_t bytes_digest, uint8_t *array_salt,  //
    ...
    uint8_t f11_8[mvl_SNOVA * vl_SNOVA] __attribute__((aligned(32))) = {0};
        ...
        for (int jdx = 0; jdx < v_SNOVA; jdx++)
            for (int kdx = 0; kdx < v_SNOVA; kdx++)
                for (int k1 = 0; k1 < l_SNOVA; ++k1) {
                    uint32_t *target =
                        (uint32_t *)&f11_8[jdx * l_SNOVA * mvl_SNOVA + k1 * mvl_SNOVA + mi * v_SNOVA * l_SNOVA + kdx * l_SNOVA];
    ...
    for (int mi = 0; mi < m_SNOVA; mi++)
        for (int jdx = 0; jdx < v_SNOVA; jdx++)
            for (int kdx = 0; kdx < o_SNOVA; kdx++) {
                ...
                for (int idx = 0; idx < l_SNOVA; idx++) pres[idx * mol_SNOVA / 4] = tres[idx];
            }
    ...
    do {
        ...
        uint8_t temp_l8[mol_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};
        uint8_t temp_r8[mol_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};

        uint8_t temp_left8[mol_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};
        uint8_t temp_right8[mol_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};
        uint8_t gtemp8[mol_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};
        uint8_t fvv_temp8[ml_SNOVA * v_SNOVA * lcube_SNOVA] __attribute__((aligned(32))) = {0};
                    ...
                    for (int jdx = 0; jdx < v_SNOVA; jdx++) {
                        ...
                        for (int mi_kdx_j1 = 0; mi_kdx_j1 < mvl_SNOVA32; mi_kdx_j1++)
                            temp3_256[alpha_i1 * mvl_SNOVA32 + mi_kdx_j1] ^=
                                _mm256_shuffle_epi8(k_lh, f11_256[jdx_k1 * mvl_SNOVA32 + mi_kdx_j1]);
                    }
            ...
            for (int mi = 0; mi < m_SNOVA; mi++)
                for (int kdx = 0; kdx < v_SNOVA; kdx++) {
                    ...
                    for (int j1 = 0; j1 < l_SNOVA; j1++)
                        fvv_temp32[(alpha * l_SNOVA * v_SNOVA * ml_SNOVA + j1 * v_SNOVA * ml_SNOVA + kdx * ml_SNOVA +
                                    mi * l_SNOVA) /
                                   4] = tres[j1];
                }
                    ...
                    for (int k1 = 0; k1 < l_SNOVA; ++k1) {
                        ...
                        for (int mi_i1 = 0; mi_i1 < ml_SNOVA32; mi_i1++)
                            fvv_res256[j1 * ml_SNOVA32 + mi_i1] ^=
                                _mm256_shuffle_epi8(k_lh, fvv_temp256[alpha * l_SNOVA * v_SNOVA * ml_SNOVA32 +
                                                                      k1 * v_SNOVA * ml_SNOVA32 + jdx * ml_SNOVA32 + mi_i1]);
                    }
                ...
                for (int jdx = 0; jdx < v_SNOVA; jdx++)
                    for (int k1 = 0; k1 < l_SNOVA; k1++) {
                        ...
                        for (int mi_kdx_j1 = 0; mi_kdx_j1 < mol_SNOVA32; mi_kdx_j1++)
                            temp_l256[(alpha * l_SNOVA + i1) * mol_SNOVA32 + mi_kdx_j1] ^=
            ...
            for (int i1 = 0; i1 < l_SNOVA; i1++)
                for (int jdx = 0; jdx < v_SNOVA; jdx++)
                    for (int k1 = 0; k1 < l_SNOVA; k1++) {
                        ...
                        for (int mi_kdx_j1 = 0; mi_kdx_j1 < mol_SNOVA32; mi_kdx_j1++)
                            temp_r256[alpha_i1 * mol_SNOVA32 + mi_kdx_j1] ^=
        ...
        for (int alpha = 0; alpha < lsq_SNOVA; ++alpha)
            for (int mi = 0; mi < m_SNOVA; ++mi)
                for (int kdx = 0; kdx < o_SNOVA; ++kdx) {
                    uint32_t *res32 =
                        (uint32_t *)&temp_l8[alpha * l_SNOVA * mol_SNOVA + mi * o_SNOVA * l_SNOVA + kdx * l_SNOVA];
                    ...
                    uint32_t *temp_left32 =
                        (uint32_t *)&temp_left8[alpha * l_SNOVA * mol_SNOVA + mi * o_SNOVA * l_SNOVA + kdx * l_SNOVA];

                    for (int j1 = 0; j1 < l_SNOVA; j1++) temp_left32[j1 * mol_SNOVA / 4] = tres[j1];
                }
        ...
        for (int alpha = 0; alpha < lsq_SNOVA; ++alpha)
            for (int mi = 0; mi < m_SNOVA; ++mi)
                for (int kdx = 0; kdx < o_SNOVA; ++kdx) {
                    uint32_t *res32 =
                        (uint32_t *)&temp_r8[alpha * l_SNOVA * mol_SNOVA + mi * o_SNOVA * l_SNOVA + kdx * l_SNOVA];
                    __m128i val128 = _mm_setr_epi32(res32[0 * mol_SNOVA / 4], res32[1 * mol_SNOVA / 4],
                                                    res32[2 * mol_SNOVA / 4], res32[3 * mol_SNOVA / 4]);
                    uint32_t tres[4];
                    _mm_storeu_si128((__m128i_u *)&tres, _mm_shuffle_epi8(val128, transpose4x4));
                    ...
                    uint32_t *temp_left32 =
                        (uint32_t *)&temp_right8[alpha * l_SNOVA * mol_SNOVA + mi * o_SNOVA * l_SNOVA + kdx * l_SNOVA];
                    for (int j1 = 0; j1 < l_SNOVA; j1++) {
        ...
        for (int mi2 = 0; mi2 < m_SNOVA * lsq_SNOVA; ++mi2) {
            ...
            for (int k = kstart; k < GAUSS_ROW_mult32; k += 32) gf16_32_mul_k(Gauss[mi2] + k, t_GF16, Gauss[mi2] + k);
    ...
    for (int mil2 = m_SNOVA * lsq_SNOVA - 1; mil2 >= 0; --mil2) {
        ...
        for (int k2 = 0; k2 < 32; k2++) t_GF16 ^= t_GF16_256_8_ptr[k2];
    ...
    for (int i = 0; i < o_SNOVA; ++i)
        for (int j = 0; j < v_SNOVA; ++j) T12_u64_ov[i][j] = T12_u64[j][i];
