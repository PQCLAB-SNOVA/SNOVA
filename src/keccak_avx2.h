 ...
 * SNOVA 2025
 *
 * Originally KeccakP-1600-times4-SIMD256.c from XKCP
 *
 * Edited #include's to be a single file
 */
...
static void KeccakP1600times4_InitializeAll(KeccakP1600times4_states *states);
...
static void KeccakP1600times4_AddBytes(KeccakP1600times4_states *states, unsigned int instanceIndex, const unsigned char *data,
                                       unsigned int offset, unsigned int length);
static void KeccakP1600times4_AddLanesAll(KeccakP1600times4_states *states, const unsigned char *data, unsigned int laneCount,
        unsigned int laneOffset);
static void KeccakP1600times4_OverwriteBytes(KeccakP1600times4_states *states, unsigned int instanceIndex,
        const unsigned char *data, unsigned int offset, unsigned int length);
static void KeccakP1600times4_OverwriteLanesAll(KeccakP1600times4_states *states, const unsigned char *data,
        unsigned int laneCount, unsigned int laneOffset);
static void KeccakP1600times4_OverwriteWithZeroes(KeccakP1600times4_states *states, unsigned int instanceIndex,
        unsigned int byteCount);
static void KeccakP1600times4_PermuteAll_4rounds(KeccakP1600times4_states *states);
static void KeccakP1600times4_PermuteAll_6rounds(KeccakP1600times4_states *states);
static void KeccakP1600times4_PermuteAll_12rounds(KeccakP1600times4_states *states);
static void KeccakP1600times4_PermuteAll_24rounds(KeccakP1600times4_states *states);
static void KeccakP1600times4_ExtractBytes(const KeccakP1600times4_states *states, unsigned int instanceIndex,
        unsigned char *data, unsigned int offset, unsigned int length);
static void KeccakP1600times4_ExtractLanesAll(const KeccakP1600times4_states *states, unsigned char *data,
        unsigned int laneCount, unsigned int laneOffset);
static void KeccakP1600times4_ExtractAndAddBytes(const KeccakP1600times4_states *states, unsigned int instanceIndex,
        const unsigned char *input, unsigned char *output, unsigned int offset,
        unsigned int length);
static void KeccakP1600times4_ExtractAndAddLanesAll(const KeccakP1600times4_states *states, const unsigned char *input,
        unsigned char *output, unsigned int laneCount, unsigned int laneOffset);
static size_t KeccakF1600times4_FastLoop_Absorb(KeccakP1600times4_states *states, unsigned int laneCount,
        unsigned int laneOffsetParallel, unsigned int laneOffsetSerial,
        const unsigned char *data, size_t dataByteLen);
static size_t KeccakP1600times4_12rounds_FastLoop_Absorb(KeccakP1600times4_states *states, unsigned int laneCount,
        unsigned int laneOffsetParallel, unsigned int laneOffsetSerial,
        const unsigned char *data, size_t dataByteLen);
static size_t KeccakP1600times4_KravatteCompress(uint64_t *xAccu, uint64_t *kRoll, const unsigned char *input,
        size_t inputByteLen);
static size_t KeccakP1600times4_KravatteExpand(uint64_t *yAccu, const uint64_t *kRoll, unsigned char *output,
...
#define UNINTLEAVE()                                                                                            \
    lanesL01 = UNPACKL(lanes0, lanes1), lanesH01 = UNPACKH(lanes0, lanes1), lanesL23 = UNPACKL(lanes2, lanes3), \
    lanesH23 = UNPACKH(lanes2, lanes3), lanes0 = PERM128(lanesL01, lanesL23, 0x20),                             \
    lanes2 = PERM128(lanesL01, lanesL23, 0x31), lanes1 = PERM128(lanesH01, lanesH23, 0x20),                     \
...
#define Xor_In4(argIndex)                                                                                                \
    lanes0 = LOAD256u(curData0[argIndex]), lanes1 = LOAD256u(curData1[argIndex]), lanes2 = LOAD256u(curData2[argIndex]), \
    lanes3 = LOAD256u(curData3[argIndex]), INTLEAVE(), XOReq256(stateAsLanes[argIndex + 0], lanes0),                     \
    XOReq256(stateAsLanes[argIndex + 1], lanes1), XOReq256(stateAsLanes[argIndex + 2], lanes2),                          \
...
#define OverWr(argIndex) \
    STORE256(stateAsLanes[argIndex], LOAD4_64(curData3[argIndex], curData2[argIndex], curData1[argIndex], curData0[argIndex]))

#define OverWr4(argIndex)                                                                                                \
    lanes0 = LOAD256u(curData0[argIndex]), lanes1 = LOAD256u(curData1[argIndex]), lanes2 = LOAD256u(curData2[argIndex]), \
    lanes3 = LOAD256u(curData3[argIndex]), INTLEAVE(), STORE256(stateAsLanes[argIndex + 0], lanes0),                     \
    STORE256(stateAsLanes[argIndex + 1], lanes1), STORE256(stateAsLanes[argIndex + 2], lanes2),                          \
...
#define Extr(argIndex)                                                                                            \
    ...
    curData2[argIndex] = stateAsLanes64[4 * (argIndex) + 2], curData3[argIndex] = stateAsLanes64[4 * (argIndex) + 3]
...
#define Extr4(argIndex)                                                                                                  \
    ...
    STORE256u(curData0[argIndex], lanes0), STORE256u(curData1[argIndex], lanes1), STORE256u(curData2[argIndex], lanes2), \
...
#define ExtrXor4(argIndex)                                                                                                     \
    ...
    lanesL23 = LOAD256u(curInput2[argIndex]), lanesH23 = LOAD256u(curInput3[argIndex]), XOReq256(lanes0, lanesL01),            \
    XOReq256(lanes1, lanesH01), XOReq256(lanes2, lanesL23), XOReq256(lanes3, lanesH23),                                        \
    STORE256u(curOutput0[argIndex], lanes0), STORE256u(curOutput1[argIndex], lanes1), STORE256u(curOutput2[argIndex], lanes2), \
...
#define rounds24                                                                                                             \
    prepareTheta thetaRhoPiChiIotaPrepareTheta(0, A, E) thetaRhoPiChiIotaPrepareTheta(1, E, A)                               \
        thetaRhoPiChiIotaPrepareTheta(2, A, E) thetaRhoPiChiIotaPrepareTheta(3, E, A) thetaRhoPiChiIotaPrepareTheta(4, A, E) \
            thetaRhoPiChiIotaPrepareTheta(5, E, A) thetaRhoPiChiIotaPrepareTheta(6, A, E)                                    \
                                ...
                                thetaRhoPiChiIotaPrepareTheta(15, E, A) thetaRhoPiChiIotaPrepareTheta(16, A, E)              \
                                    thetaRhoPiChiIotaPrepareTheta(17, E, A) thetaRhoPiChiIotaPrepareTheta(18, A, E)          \
                                        thetaRhoPiChiIotaPrepareTheta(19, E, A) thetaRhoPiChiIotaPrepareTheta(20, A, E)      \
                                            thetaRhoPiChiIotaPrepareTheta(21, E, A) thetaRhoPiChiIotaPrepareTheta(22, A, E)  \
...
#define rounds6                                                                                  \
    prepareTheta thetaRhoPiChiIotaPrepareTheta(18, A, E) thetaRhoPiChiIotaPrepareTheta(19, E, A) \
        thetaRhoPiChiIotaPrepareTheta(20, A, E) thetaRhoPiChiIotaPrepareTheta(21, E, A)          \
            thetaRhoPiChiIotaPrepareTheta(22, A, E) thetaRhoPiChiIota(23, E, A)

#define rounds4                                                                                  \
    prepareTheta thetaRhoPiChiIotaPrepareTheta(20, A, E) thetaRhoPiChiIotaPrepareTheta(21, E, A) \
        thetaRhoPiChiIotaPrepareTheta(22, A, E) thetaRhoPiChiIota(23, E, A)
                ...
                thetaRhoPiChiIotaPrepareTheta(i + 4, A, E) thetaRhoPiChiIotaPrepareTheta(i + 5, E, A)               \
                    thetaRhoPiChiIotaPrepareTheta(i + 6, A, E) thetaRhoPiChiIotaPrepareTheta(i + 7, E, A)           \
                        thetaRhoPiChiIotaPrepareTheta(i + 8, A, E) thetaRhoPiChiIotaPrepareTheta(i + 9, E, A)       \
                            thetaRhoPiChiIotaPrepareTheta(i + 10, A, E) thetaRhoPiChiIotaPrepareTheta(i + 11, E, A) \
    }
...
#define rounds6                                                                           \
    prepareTheta for (i = 18; i < 24; i += 2) {                                           \
        thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
    }
...
#define rounds6                                                                           \
    prepareTheta for (i = 18; i < 24; i += 3) {                                           \
        thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
            thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) copyStateVariables(A, E)           \
    }

#define rounds4                                                                           \
    prepareTheta for (i = 20; i < 24; i += 2) {                                           \
        thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
    }
...
#elif (Unrolling == 1)
#define rounds24 \
    prepareTheta for (i = 0; i < 24; i++) { thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) }

#define rounds12 \
    prepareTheta for (i = 12; i < 24; i++) { thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) }

#define rounds6 \
    prepareTheta for (i = 18; i < 24; i++) { thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) }

#define rounds4 \
    prepareTheta for (i = 20; i < 24; i++) { thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) }
...
#define LoadXOReq256(lanes, inp, argIndex) \
    XOReq256(lanes, LOAD4_64(inp[3 * 25 + argIndex], inp[2 * 25 + argIndex], inp[1 * 25 + argIndex], inp[0 * 25 + argIndex]))
...
#define AddOverWr4(lanes0, lanes1, lanes2, lanes3, key, inp, argIndex)                                  \
    ...
    XOReq256(lanes0, CONST256_64(key[argIndex + 0])), XOReq256(lanes1, CONST256_64(key[argIndex + 1])), \
    XOReq256(lanes2, CONST256_64(key[argIndex + 2])), XOReq256(lanes3, CONST256_64(key[argIndex + 3]))

#else

#define AddOverWr4(lanes0, lanes1, lanes2, lanes3, key, inp, argIndex)                                  \
    lanes0 = LOAD256u(inp[argIndex + 0 * 25]), lanes1 = LOAD256u(inp[argIndex + 1 * 25]),               \
    lanes2 = LOAD256u(inp[argIndex + 2 * 25]), lanes3 = LOAD256u(inp[argIndex + 3 * 25]),               \
    INTLEAVEa(lanes0, lanes1, lanes2, lanes3), XOReq256(lanes0, CONST256_64(key[argIndex + 0])),        \
    XOReq256(lanes1, CONST256_64(key[argIndex + 1])), XOReq256(lanes2, CONST256_64(key[argIndex + 2])), \
...
#define ExtrAccu(lanes, p, argIndex)                                                                                  \
    p[argIndex] ^= _mm256_extract_epi64(lanes, 0) ^ _mm256_extract_epi64(lanes, 1) ^ _mm256_extract_epi64(lanes, 2) ^ \
                   _mm256_extract_epi64(lanes, 3)

#define ExtrAccu4(lanes0, lanes1, lanes2, lanes3, p, argIndex)                                       \
    UNINTLEAVEa(lanes0, lanes1, lanes2, lanes3), XOReq256(lanes0, lanes1), XOReq256(lanes2, lanes3), \
        lanes1 = LOAD256u(p[argIndex]), XOReq256(lanes0, lanes2), XOReq256(lanes0, lanes1), STORE256u(p[argIndex], lanes0)

#define Kravatte_Rollc()                                                                                           \
    Asa = x0x1x2x3, Ase = x1x2x3x4, ROL64in256(x1x2x3x4, x0x1x2x3, 7), XOReq256(x1x2x3x4, Ase),                    \
...
#define ExtrAddKey(lanes, p, argIndex)                                                                                     \
    XOReq256(lanes, CONST256_64(kRoll[argIndex])), p[argIndex + 0 * 25] = _mm256_extract_epi64(lanes, 0),                  \
...
#define ExtrAddKey4(lanes0, lanes1, lanes2, lanes3, p, argIndex)                                                \
    XOReq256(lanes0, CONST256_64(kRoll[argIndex + 0])), XOReq256(lanes1, CONST256_64(kRoll[argIndex + 1])),     \
        XOReq256(lanes2, CONST256_64(kRoll[argIndex + 2])), XOReq256(lanes3, CONST256_64(kRoll[argIndex + 3])), \
...
#define ExtrAddKey4(lanes0, lanes1, lanes2, lanes3, p, argIndex)                                                \
    XOReq256(lanes0, CONST256_64(kRoll[argIndex + 0])), XOReq256(lanes1, CONST256_64(kRoll[argIndex + 1])),     \
        XOReq256(lanes2, CONST256_64(kRoll[argIndex + 2])), XOReq256(lanes3, CONST256_64(kRoll[argIndex + 3])), \
        UNINTLEAVEa(lanes0, lanes1, lanes2, lanes3), STORE256u(p[argIndex + 0 * 25], lanes0),                   \
        STORE256u(p[argIndex + 1 * 25], lanes1), STORE256u(p[argIndex + 2 * 25], lanes2),                       \
