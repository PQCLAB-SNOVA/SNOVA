...
static inline void aes128ni_encrypt_x4(const __m128i rkeys[15], __m128i n0, __m128i n1, __m128i n2, __m128i n3, unsigned char *out) {
...
void oqs_aes128_ctr_enc_sch_ni(const uint8_t *iv, const size_t iv_len, const void *schedule, uint8_t *out, size_t out_len) {
	...
	while (out_len >= 16) {
		aes128ni_encrypt(schedule, (unsigned char *)&block, out);
		out += 16;
		out_len -= 16;
	...
	if (out_len > 0) {
		uint8_t tmp[16];
		aes128ni_encrypt(schedule, (unsigned char *)&block, tmp);
