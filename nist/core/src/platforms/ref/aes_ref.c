...
void aes256_keyexp_ref(const uint8_t key[32], uint8_t rk[240]) {
    ...
    for (int i = 32; i < 240; i += 4) {
        ...
        rk[i+0] = rk[i-32+0] ^ t[0];
        rk[i+1] = rk[i-32+1] ^ t[1];
        rk[i+2] = rk[i-32+2] ^ t[2];
        rk[i+3] = rk[i-32+3] ^ t[3];
    }
}
...
void aes128_ctr_zero_ref(uint8_t *out, size_t outlen, const uint8_t key[16]) {
    ...
    uint8_t ctr[16] = {0};
    uint8_t block[16];
    aes128_keyexp_ref(key, rk);
    while (outlen > 0) {
        aes128_encrypt_ref(rk, ctr, block);
        size_t n = (outlen > 16) ? 16 : outlen;
        memcpy(out, block, n);
        out += n;
        outlen -= n;
