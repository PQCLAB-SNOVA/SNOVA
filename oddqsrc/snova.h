...
#ifdef SYMMETRIC
#define NUMGF_PK (SNOVA_m * SNOVA_o * SNOVA_l * (SNOVA_o * SNOVA_l + 1) / 2)
#else
...
#else
#define BYTES_HASH (GF16_HASH / 2)
#endif

#define SNOVA_alpha (SNOVA_l * SNOVA_l + SNOVA_l)
#ifdef SYMMETRIC
#define NUM_GEN_PUB_GF                                                                                            \
    ((SNOVA_m * (SNOVA_v * (SNOVA_v + 1) / 2 + 2 * SNOVA_v * SNOVA_o) + 2 * (SNOVA_o * SNOVA_alpha)) * SNOVA_l2 + \
     2 * SNOVA_o * SNOVA_alpha * SNOVA_l)
#else
#define NUM_GEN_PUB_GF                                                                                  \
    ((SNOVA_m * (SNOVA_v * SNOVA_v + 2 * SNOVA_v * SNOVA_o) + 2 * (SNOVA_o * SNOVA_alpha)) * SNOVA_l2 + \
     2 * SNOVA_o * SNOVA_alpha * SNOVA_l)
#endif
#define NUM_PUB_GF                                                                                      \
    ((SNOVA_m * (SNOVA_v * SNOVA_v + 2 * SNOVA_v * SNOVA_o) + 2 * (SNOVA_o * SNOVA_alpha)) * SNOVA_l2 + \
...
#define NUM_GEN_SEC_BYTES (SNOVA_v * SNOVA_l2 / 2)
#endif

#define i_prime(mi, alpha) ((alpha + mi) % SNOVA_m)

typedef struct {
	uint16_t P11[SNOVA_m * SNOVA_n * SNOVA_n * SNOVA_l2];
	uint16_t T12[SNOVA_o * SNOVA_v * SNOVA_l2];
	uint16_t F21[SNOVA_m * SNOVA_o * SNOVA_v * SNOVA_l2];
#ifndef SYMMETRIC
	uint16_t F12[SNOVA_m * SNOVA_o * SNOVA_v * SNOVA_l2];
#endif
	uint16_t Am[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint16_t Bm[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint16_t Q1[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint16_t Q2[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint16_t q1[SNOVA_m * SNOVA_alpha * SNOVA_l];
	uint16_t q2[SNOVA_m * SNOVA_alpha * SNOVA_l];
	uint8_t sk_seed[SEED_LENGTH];
} expanded_SK;

typedef struct {
	uint16_t P[SNOVA_m * SNOVA_n * SNOVA_n * SNOVA_l2];
	uint8_t Am[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint8_t Bm[SNOVA_m * SNOVA_alpha * SNOVA_l2];
	uint8_t q1[SNOVA_m * SNOVA_alpha * SNOVA_l];
	uint8_t q2[SNOVA_m * SNOVA_alpha * SNOVA_l];
