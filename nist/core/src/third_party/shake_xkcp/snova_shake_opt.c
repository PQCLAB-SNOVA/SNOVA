...
#if PARALLELISM == 1
        KeccakP1600_Permute_24rounds((KeccakP1600_plain64_state *)states);
#elif PARALLELISM == 4
        KeccakP1600times4_PermuteAll_24rounds((KeccakP1600times4_SIMD256_states *)states);
#elif PARALLELISM == 8
        KeccakP1600times8_PermuteAll_24rounds((KeccakP1600times8_SIMD512_states *)states);
#else
...
/*
: init replicates snova_shake_squeeze's per-lane prepared_state setup
 * (:51-61) into a persistent ctx; next is a single iteration of its while
 * loop (:63-101), always emitting the full rate for every lane (the caller
 * truncates). Prefix property of the CTR-XOF gives byte-identity. 
 */
                             ...
                             size_t input_bytes)
{
    ...
    KeccakWidth1600_SpongeInstance *sponge = &keccak_instance.sponge;
    uint8_t *prepared_state = (uint8_t *)ctx->prepared64;
    uint32_t bytes_rate = sponge->rate / 8;

    for (int idx = 0; idx < PARALLELISM; idx++)
    {
        uint64_t *state64 = (uint64_t *)sponge->state.A;
        uint64_t *prep64 = ctx->prepared64;
    ...
    ctx->block = 0;
    ctx->byteIOIndex = sponge->byteIOIndex;
    ctx->bytes_rate = bytes_rate;
}
...
size_t snova_shake_stream_next(snova_xof_stream_t *ctx, uint8_t *out)
{
    ...
    const uint32_t bytes_rate = ctx->bytes_rate;

    memcpy(states, ctx->prepared64, PARALLELISM * 200);
    for (int idx = 0; idx < PARALLELISM; idx++)
    {
        states64[ctx->byteIOIndex * PARALLELISM / 8 + idx] ^= ctx->block;
        ctx->block++;
    }
...
#error "PARALLELISM must be 1, 4 or 8"
    ...
    {
        uint64_t *source64 = states64 + idx;
        for (size_t idx2 = 0; idx2 < bytes_rate / 8; idx2++)
        {
            *out64 = *source64;
            source64 += PARALLELISM;
            out64++;
    ...
    return (size_t)PARALLELISM * bytes_rate;
}
#endif /* SNOVA_VERIFY_STREAM || SNOVA_SIGN_STREAM */
