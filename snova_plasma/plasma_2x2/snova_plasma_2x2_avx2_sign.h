#ifndef PLASMA_2x2_AVX2_SIGN_H
#define PLASMA_2x2_AVX2_SIGN_H
 ...
 * Computes signature
 */
int sign_digest_core_2x2_avx2_vtl(uint8_t *pt_signature, const uint8_t *digest,
                         ...
                         const uint8_t pt_private_key_seed[seed_length_private])
{
    ...
    do
    {
        ...
        uint8_t left8[vl4_SNOVA32 * 32];
        uint8_t right8[vl4_SNOVA32 * 32];
        ...
        for (int alpha_i1 = 0; alpha_i1 < lsq_SNOVA; alpha_i1++)
            for (int jdx_k1 = 0; jdx_k1 < vl_SNOVA; jdx_k1++)
            {
                __m256i k_lh = vtl_multtab(left8[alpha_i1 * 2 * vl_SNOVA + jdx_k1]) ^
                               _mm256_slli_epi16(vtl_multtab(left8[alpha_i1 * 2 * vl_SNOVA + jdx_k1 + vl_SNOVA]), 4);

                for (int mi_kdx_j1 = 0; mi_kdx_j1 < mvl_SNOVA32; mi_kdx_j1++)
                    temp3_256[alpha_i1 * 2 * mvl_SNOVA32 + mi_kdx_j1] ^= _mm256_shuffle_epi8(k_lh, f11_256[jdx_k1 * mvl_SNOVA32 + mi_kdx_j1]);
            }
        ...
        for (int kdx = 0; kdx < v_SNOVA; kdx++)
            for (int mi = 0; mi < m_SNOVA; mi++)
                for (int alpha = 0; alpha < lsq_SNOVA; ++alpha)
                {
                    uint16_t val0 = *(uint16_t *)&temp3_8[alpha * l_SNOVA * mvl_SNOVA + 0 * mvl_SNOVA + mi * v_SNOVA * l_SNOVA + kdx * l_SNOVA];
                    uint16_t val1 = *(uint16_t *)&temp3_8[alpha * l_SNOVA * mvl_SNOVA + 1 * mvl_SNOVA + mi * v_SNOVA * l_SNOVA + kdx * l_SNOVA];
                    *(uint16_t *)&fvv_temp8[alpha * l_SNOVA * v_SNOVA * ml_SNOVA + 0 * v_SNOVA * ml_SNOVA + kdx * ml_SNOVA + mi * l_SNOVA] = (val0 & 0xf) ^ ((val1 << 8) & 0xf00);
                    *(uint16_t *)&fvv_temp8[alpha * l_SNOVA * v_SNOVA * ml_SNOVA + 1 * v_SNOVA * ml_SNOVA + kdx * ml_SNOVA + mi * l_SNOVA] = ((val0 >> 8) & 0xf) ^ (val1 & 0xf00);
                }
